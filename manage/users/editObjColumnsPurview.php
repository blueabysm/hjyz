...
include("editObjColumnsPurviewClass.inc.php");

$myPageClass = new editObjColumnsPurviewClass($_GET,$mysqldao);
...
<table id="FormTable" cellSpacing="1" cellPadding="0" width="98%"
	...
	<tr>
		<td class="FormCaption" align="center" bgColor="#668cd9">该用户能管理以下栏目</td>
	</tr>
	<tr>
		...
		<table height="99%" cellSpacing="1" cellPadding="0" width="100%"
			...
			<TR>
				<TD class="FormLabel" align="left" colspan="2">
					<a href='addColumnsPurview.php?page=<?=$myPageClass->page?>&uid=<?=$myPageClass->uid?>&t=<?=$myPageClass->t?>&id=<?=$myPageClass->id?>'>增加栏目管理权</a>
			...
			<tr>
				...
				<TABLE cellSpacing="1" cellPadding="1" width="100%"	bgColor="#ffffff" border="0">
					<tr>
						...
						<td class="TableTdCaption" width="20%" align="center">栏目名称</td>
					...
					if ($myPageClass->objList != -1)
					{
						for ($i=0;$i<count($myPageClass->objList);$i++)
						{
							...
							echo "</td>\n<td class='TableTd' align='center' $tmpstr>";
							echo "<a href='editColumnsPurview.php?t=".$myPageClass->t."&rec_id=".$myPageClass->objList[$i]["rec_id"]."&uid=".$myPageClass->uid."&page=".$myPageClass->page."&id=$myPageClass->id'>修改</a> ";
							echo "<a onclick=\"return confirm('确定要删除对此栏目的管理权？')\" href='delColumnsPurview.php?t=".$myPageClass->t."&rec_id=".$myPageClass->objList[$i]["rec_id"]."&uid=".$myPageClass->uid."&page=".$myPageClass->page."&id=$myPageClass->id'>删除</a> ";
							echo "</td>\n</tr>";						
						}
					}					   
					?>					
