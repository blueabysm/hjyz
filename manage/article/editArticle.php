	...
	<br>
		<form id="editArticle" name="editArticle" onsubmit="return GetData();"  method="post">
			<table id="FormTable" cellSpacing="1" cellPadding="0" width="98%" align="center" bgColor="#ffffff"
				border="0">
				<tr>
					<td class="FormCaption" align="center" bgColor="#668cd9">添加/修改文章</td>
				</tr>
				<tr>
					<td vAlign="middle" align="center" bgColor="#f1f3f5">
						<table cellSpacing="1" cellPadding="0" width="99%" align="center" bgColor="#ebebeb"
							...
							<TR>
								...
								<a href='manageArticle.php?id=<?=$myPageClass->item_id?>&page=<?echo $myPageClass->to_page?>&retURL=<?=$myPageClass->retURL?>'>返回文章列表</a>								
								</TD>
							</TR>
							<TR>
							...
							<TR>
								<TD class="FormLabel" align="right">评论状态:</TD>
								<TD class="FormLabel">
									<select name="comments_type" id="comments_type">
						     			<?php
						     			  for($i=0;$i<count($myPageClass->comments_type_list);$i+=2)
						     			  {
						     			  	if ($myPageClass->comments_type_list[$i] == $myPageClass->comments_type){
						     			  		echo "<option value='".$myPageClass->comments_type_list[$i]."' selected='selected'>".$myPageClass->comments_type_list[$i+1]."</option>\n";
						     			  	} else {
						     			  		echo "<option value='".$myPageClass->comments_type_list[$i]."'>".$myPageClass->comments_type_list[$i+1]."</option>\n";     			  		
						     			  	}
						     			  } 
						     			?>
						     		</select>
