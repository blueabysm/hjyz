...
class editArticleClass{
	...
	{	
		...
		//检查权限
		$this->my_purviews = checkPurview(34,$this->item_id,$this->mysql,array('add','edit','editAll'));
		if ($this->my_purviews == -1){
			$this->errorMessage = "没有权限!";
			return;
		}
		
		$sqlstr = "select columns_name from columns where columns_id=$this->item_id and $this->item_id in (select obj_id from my_object where menu_id=34 and user_id=".$_SESSION["sess_user_id"].")";
	...
	function btnSave_Click()
	{
		...
		$this->comments_type = trim($this->postVars["comments_type"]);
